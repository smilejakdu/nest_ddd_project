...
describe('Result', () => {
	...
	it('isSuccess 가 True 이면서 error 도 있는 경우의 예외케이스 확인', () => {
		try {
			...
			expect(error.message).toEqual('InvalidOperation : A result can`t be successful and contain an error.');
		}
	});
	...
	it('isSuccess 가 False 이면서 error 도 없는 경우의 예외케이스 확인', () => {
		try {
			...
			expect(error.message).toEqual('InvalidOperation : A failing result needs to contain an error message.');
		}
	});
	...
	it('Result 가 생성된 후에는 isSuccess(isFailure, error) 가 변할 수 없는 지', () => {
		try {
			...
			expect(error.message).toEqual("Cannot assign to read only property 'isSuccess' of object '#<Result>'");
		}
	});
	...
	it('value get 메소드는 isSuccess 가 True 인 경우에만 가능한지', () => {
		try {
			...
			expect(error.message).toEqual('Can`t get the value of an error result. Use `errorValue` instead.');
		}
	});
	...
	it('errorValue 메소드는 isSuccess 가 False 인 경우에만 가능한지', () => {
		try {
			...
			expect(error.message).toEqual('Can`t execute errorValue of an success result. Use `value` instead.');
		}
	});
	...
	it('successResult 여러개가 들어온 경우에 combine 메소드는 Result.ok() 를 리턴하는지', () => {
		const results = [new Result(true, undefined, VALUE_OK), new Result(true, undefined, 'VALUE_OK_2')];
